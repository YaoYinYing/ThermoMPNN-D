...
from thermompnn.model.modules import (LightAttention, MPNNLayer,
                                      SideChainModule, get_protein_mpnn)
...
def _check_sequence_match(S, wt, mut, pos):
    ...
    for mut_idx in range(wt.shape[-1]):  # check each mutation separately
        ...
        if torch.sum(S_check - wt[..., mut_idx]) != 0:  # if matched, keep S values
            S_check = torch.gather(S, -1, pos[..., mut_idx, None])  # selects all amino acids in seq at pos locations
            S.scatter_(dim=1, index=pos[..., mut_idx][..., None], src=wt[..., mut_idx]
                       [..., None])  # scatter wt values into S at specified positions
            S_check = torch.gather(S, -1, pos[..., mut_idx, None])  # selects all amino acids in seq at pos locations
    return S
...
class TransferModelv2(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.cfg.model.lightattn:
            if self.multi_mutations:
                ...
                self.light_attention.append(
                    nn.LayerNorm(
                        HIDDEN_DIM *
                        self.cfg.model.num_final_layers +
                        EMBED_DIM))  # do layer norm before MLP
                self.light_attention.append(
                    nn.Linear(
                        HIDDEN_DIM *
                        self.cfg.model.num_final_layers +
                        EMBED_DIM,
                        HIDDEN_DIM,
                        bias=True))  # E down to 128 regardless
    ...
    def forward(
        ...
        if self.cfg.model.side_chain_module:
            all_mpnn_hid, mpnn_embed, _, mpnn_edges = self.prot_mpnn(
                X[:, :, :4, :], S, mask, chain_M, residue_idx, chain_encoding_all)
        else:
            all_mpnn_hid, mpnn_embed, _, mpnn_edges = self.prot_mpnn(
        ...
        if self.multi_mutations:
            if self.cfg.model.side_chain_module:
                side_chain_embeds = self.side_chain_features(
                    X, S, mask, chain_M, residue_idx, chain_encoding_all, all_mpnn_hid[0], atom_mask)
            ...
            if self.cfg.model.num_final_layers > 0:
                ...
                if self.cfg.model.mutant_embedding:
                    ...
                    mut_embed = torch.cat([m.unsqueeze(-1) for m in mut_embed_list], -
                                          1)  # shape: (Batch, Embed, N_muts)
                ...
                if self.cfg.model.edges:  # add edges to input for gathering
                    ...
                    for n_current in n_mutations:  # iterate over N-order mutations
                        ...
                        mpnn_edges_tmp = torch.squeeze(batched_index_select(
                            mpnn_edges, 1, mut_positions[:, n_current:n_current + 1]), 1)
                        E_idx_tmp = torch.squeeze(batched_index_select(
                            E_idx, 1, mut_positions[:, n_current:n_current + 1]), 1)
                        ...
                        for b in range(E_idx_tmp.shape[0]):
                            ...
                            tmp_edges = []
                            for n_o in n_other:
                                idx = torch.where(E_idx_tmp[b, :] == mut_positions[b,
                                                  n_o:n_o + 1].expand(1, E_idx_tmp.shape[-1]))
            ...
            for i in range(mut_mutant_AAs.shape[-1]):
                ...
                gathered_embed = torch.gather(mpnn_embed,
                                              1,
                ...
                if self.cfg.model.side_chain_module:
                    side_chain_gathered = torch.gather(side_chain_embeds,
                                                       1,
                                                       current_positions.unsqueeze(-1).expand(current_positions.size(0),
                                                                                              current_positions.size(1),
                                                                                              side_chain_embeds.size(2)))
            ...
            mpnn_embed = torch.gather(mpnn_embed,
                                      1,
                                      mut_positions.unsqueeze(-1).expand(mut_positions.size(0),
                                                                         mut_positions.size(1),
                                                                         mpnn_embed.size(2)))
...
class TransferModelv2Siamese(nn.Module):
    ...
    def forward(
        ...
        for i in range(mut_mutant_AAs.shape[-1]):
            ...
            g_struct_embed = torch.gather(all_mpnn_hid,
                                          1,
                                          current_positions.unsqueeze(-1).expand(current_positions.size(0),
                                                                                 current_positions.size(1),
                                                                                 all_mpnn_hid.size(2)))
            ...
            # add specific mutant embedding to gathered embed based on which mutation is being gathered
            g_seq_embed = torch.gather(wt_embed,
                                       1,
                                       current_positions.unsqueeze(-1).expand(current_positions.size(0),
                                                                              current_positions.size(1),
                                                                              wt_embed.size(2)))
        ...
        # if batch is double-padded, check and zero out any singles in second half of embedding
            ...
            # [B, ] 0 if single, 1 if double
            single_mask = ~torch.logical_and(mut_wildtype_AAs[..., -1] == 0, mut_mutant_AAs[..., -1] == 0)
