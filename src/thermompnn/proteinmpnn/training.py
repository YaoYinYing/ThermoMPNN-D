...
def main(args):
    ...
    with ProcessPoolExecutor(max_workers=12) as executor:
        ...
        for e in range(args.num_epochs):
            ...
            checkpoint_filename_last = base_folder + f'model_weights/epoch_last.pt'
            torch.save({
                'epoch': e + 1,
                'step': total_step,
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--use_ipmp",
        action='store_true',
        help="use ipmp layers instead of mpnn layers for encoding and decoing")
    argparser.add_argument("--n_points", type=int, default=8, help="number of points for IPMP")
    argparser.add_argument("--rescut", type=float, default=3.5, help="PDB resolution cutoff")
    argparser.add_argument("--debug", type=bool, default=False, help="minimal data loading for debugging")
    argparser.add_argument("--gradient_norm", type=float, default=-
                           1.0, help="clip gradient norm, set to negative to omit clipping")
    ...
    argparser.add_argument(
        ...
        type=bool,
        default=False,
        help='train on FULL dataset (train+val+test set) to replicate literature results')
