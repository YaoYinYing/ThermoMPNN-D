...
def inference(cfg, args):
    ...
    if cfg.model.aggregation == 'siamese':
        model = TransferModelPLv2Siamese.load_from_checkpoint(
            args.model, cfg=cfg, map_location=device, train_dataset=ds, val_dataset=ds).model
    else:
        model = TransferModelPLv2.load_from_checkpoint(
            args.model, cfg=cfg, map_location=device, train_dataset=ds, val_dataset=ds).model
